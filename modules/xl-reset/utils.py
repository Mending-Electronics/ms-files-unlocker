import os
from modules.core.utils import setup_directories, extract_excel_file, repackage_excel_file, remove_worksheet_password, remove_workbook_password, reset_work
from modules.core.logging_config import configure_module_logger

# Configure logger for xl-reset module
    ...
    try:
        ...
        if os.path.exists(worksheets_dir):
            for worksheet_file in os.listdir(worksheets_dir):
                ...
                logger.debug(f"Processing worksheet: {worksheet_file}")
                remove_worksheet_password(worksheet_path, zip_temp, dir_temp)
        
        # Process workbook
        workbook_path = os.path.join(dir_temp, 'xl', 'workbook.xml')
        if os.path.exists(workbook_path):
            logger.debug("Processing workbook.xml")
            remove_workbook_password(workbook_path, zip_temp, dir_temp)
